...
class DemoClientPeer(AbstractPeer):
    def __init__(self):
        if not os.path.exists('from_server'):
            with open('from_server', 'w') as f:
                pass

        self.input_stream = open('from_server', 'rb+')
        self.input_stream.read() # Move to the end of the stream to ignore old messages
        self.output_stream = open('from_client', 'ab+')
...
class DemoClient(AbstractP2PClient):
    ...
    def on_file(self, filename, file_data):
        ...
        with open('c_transferred_' + filename, 'wb') as f:
            f.write(file_data)
